...
def blendOpenCV():
    ...
    fnameOrCvData = askopenfilename(parent=window,filetypes=(("칼라 파일", "*.jpg;*.png;*.bmp;*.tif"), ("모든파일", "*.*")))
    if fnameOrCvData == '' or fnameOrCvData == None :
        return

    ## PIL 개체 --> OpenCV 개체로 복사(이미지 추가)
    if type(fnameOrCvData) == str:  # 문자열 일때는 filename
        cvData_new = cv2.imread(fnameOrCvData)  # 파일 --> CV 개체
    else:
        cvData2_new = fnameOrCvData
    cvPhoto_new = cv2.cvtColor(cvData2_new, cv2.COLOR_BGR2RGB)  # 중요! CV 개체
    photo_new = Image.fromarray(cvPhoto_new)  # 중요! PIL 객체
    inW2 = photo_new.width
    inH2 = photo_new.height
    #################
    ## 메모리 확보 ##
    ################
    inImage2= []
    for _ in range(3):
        inImage2.append(malloc(inH, inW))
    ...
    ##이 부분이 OpenCV 처리 부분######################################
    cvPhoto2 = cvPhoto[:]
    cvPhoto3 = cvPhoto_new[:]
    width = cvPhoto2.shape[1]
    height = cvPhoto2.shape[0]
    cvPhoto3 = cv.resize(cvPho2,(width,height))
