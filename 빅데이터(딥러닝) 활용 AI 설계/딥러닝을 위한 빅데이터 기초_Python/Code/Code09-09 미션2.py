...
def lowFrequencyImage() :
    ...
    # for i in range(MSIZE//2,inH+MSIZE//2) :
    #     for k in range(MSIZE//2,inW+MSIZE//2) :
    #         alphaValue = [[num for num in range(inW+MSIZE//2)] for [num for num in range(inW+MSIZE//2)] in range(inH+MSIZE//2)]
    #         tmpOutImage[i][k] = (alphaValue * tmpInImage[i+m-MSIZE//2][k+n-MSIZE//2]) - tmpOutImage[i][k]
    # print(alpha * tmpInImage[i][k])
...
# 모핑 알고리즘
def morphImage() :
    global window, canvas, paper, filename, inImage,outImage, inW, inH, outW, outH,inImage2
    ## 중요! 코드, 출력영상 크기 결정##
    outH = inH
    outW = inW
    ##추가 영상 선택
  ...
  # 파일 --> 메모리
    ...
    ###### 메모리 할당 ################
    outImage = [];    outImage = melloc(outH, outW)
    ####### 진짜 컴퓨터 비전 알고리즘 #####
...
def saveCSV() :
    ...
    outImage = melloc(outH, outW)
    ###### 진짜 컴퓨터 비전 알고리즘 #####
    saveFp = asksaveasfile(parent=window, mode='wt',
...
def openCSV() :
    ...
    inH = inW = int(math.sqrt(len(tmp)))  # 핵심 코드 (메모리를 확보하기 위한 파일의 크기 확인)
    print(inW,inH)
...
comVisionMenu6.add_command(label='CSV 열기', command=openCSV)
