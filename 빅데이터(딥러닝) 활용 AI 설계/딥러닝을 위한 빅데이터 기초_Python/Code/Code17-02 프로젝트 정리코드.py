...
def openImageColor() :
    ...
    displayImageColor()
    # messagebox.showinfo('파일 이름', filename)
...
def addImageColor() :
    ...
    ### 진짜 컴퓨터 비전 알고리즘 ###
    ##############################
    value = askinteger("밝게/어둡게(-255 ~ 255)","값 -->",minvalue=-255,maxvalue=255)
    for RGB in range(3):
        for i in range(inH):
            for k in range(inW):
...
####################
window = Tk()
window.title('Computer Vision Tool ver 0.1')
...
comVisionMenu1.add_command(label='밝기 조정', command=addImageColor)
comVisionMenu1.add_command(label='이진화', command=binaryImageColor)
...
comVisionMenu1.add_separator()
comVisionMenu1.add_command(label='히스토그램', command=histoImage2Color)
comVisionMenu1.add_command(label='히스토그램 평활화', command=histoEqualImageColor)
comVisionMenu1.add_command(label='명암대비', command=stretchImageColor)
comVisionMenu1.add_separator()
...
mainMenu.add_cascade(label='기하학 처리', menu=comVisionMenu2)
comVisionMenu2.add_command(label='상하반전', command=upDownImageColor)
...
comVisionMenu2.add_command(label='이동', command=moveImageColor)
comVisionMenu2.add_command(label='회전2(중심, 역방향)', command=rotateImageColor2)
...
mainMenu.add_cascade(label='데이터 입출력', menu=comVisionMenu5)
